...
def parse_string(string: str):
    ...
    Args:
        string (str): _Could get 2 variations of strings:
            "Record(id=1, date=26.02.2004, content=Some example, user=1, title=Example title)"
            "User(id=1, first_name=test name, second_name=test surname, email=test@test.test, password=123)"_

    Returns:
        _dictionary_: _parsed data from sting, example: {"key":"value"}_
    """
    result = {}
    if string[0] == 'R':
        string = string[7:-1]
...
def write_to_db(string: str, database):
    """_Write to database func, not working_

    Args:
        string (str): _Could get 2 variations of strings:
            "Record(id=1, date=26.02.2004, content=Some example, user=1, title=Example title)"
            "User(id=1, first_name=test name, second_name=test surname, email=test@test.test, password=123)"_
        database (_SQLite3 database_): _Database to work with_
    """
